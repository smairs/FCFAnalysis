def find_outliers(BINFILES=['TauRelPipeline_FullResults_CRL2688_26p0to26p5_0p012to0p012_20162017.bin','TauRelPipeline_FullResults_CRL618_26p0to26p5_0p012to0p012_20162017.bin','TauRelPipeline_FullResults_URANUS_26p0to26p5_0p012to0p012_20162017.bin'],BESTKEYS=['Run_0','Run_0','Run_0'],OBSSTLIM=7,OBSENLIM=16,LOWERTAULIM=0.0,UPPERTAULIM=0.32,wavelength='450',FWHMLIM=11.0,AMLIM=5.0,PIX_SIZE=1.0,present_epoch='20180501',present_epoch_only=0,INCLUDESOURCES=['CRL2688','CRL618','URANUS','MARS','NEPTUNE']):
    ...
    #FCFarcsecupperlim = 10000
    #FCFbeamupperlim   = 10000

    sourceind_all   = []
    sourceindID_all = []
    sourceind_nogoodind_all = []
    sourceindID_nogoodind_all = []
    MJD_dates_all   = []
    MJD_dates_nogoodind_all = []
    peak_fcfs_all   = []
    peak_fcfs_nogoodind_all = []
    arcsec_fcfs_all = []
    arcsec_fcfs_nogoodind_all = []
    airmasses_all   = []
    zentrans_all    = []
    transwitham_all = []
    ATSTART_all     = []
    ...
    peakfluxes_all_norm = []
    totalfluxes_all = []
    totalfluxes_nogoodind_all =[]
    totalfluxes_nogoodind_all_norm = []
    fnames_all    = [] 
    OBSNUM_all    = [] 
    UTDATE_all    = [] 
    AZSTART_all    = [] 
    ...
    for eachresultsfile in range(len(BINFILES)):
        ...
        if str(results[BESTKEY]['source']) in INCLUDESOURCES:
            print(date_to_print+': OPACITY RELATION ('+str(results[BESTKEY]['source'])+'): tau_'+wavelength+' = '+str(results[BESTKEY]['coeff1'])+'(tau225 - '+str(results[BESTKEY]['coeff2'])+') x Airmass')
        ...
        airmasses = airmasses[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        zentrans =np.array(results[BESTKEY]['transmissions'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        totalfluxes=np.array(results[BESTKEY]['total_fluxes'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        #totalfluxes=totalfluxes1/np.average(totalfluxes1[np.where(np.isnan(totalfluxes1)*-1+1>0)])
        a = results[BESTKEY]['coeff1']                                                                                                                                   
        b = results[BESTKEY]['coeff2']                                                                                                                                                          
        trans_witham = zentrans**airmasses                                           
        ATSTART     = np.array(results[BESTKEY]['ATSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        ATEND       = np.array(results[BESTKEY]['ATEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        AMSTART     = np.array(results[BESTKEY]['AMSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        AMEND       = np.array(results[BESTKEY]['AMEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUST       = np.array(results[BESTKEY]['WVMTAUST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUEN       = np.array(results[BESTKEY]['WVMTAUEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUST_T     = np.array(results[BESTKEY]['WVMTAUST_TIME'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUEN_T     = np.array(results[BESTKEY]['WVMTAUEN_TIME'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        FWHM1s      = np.array(results[BESTKEY]['FWHM1s'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        FWHM2s      = np.array(results[BESTKEY]['FWHM2s'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        areas       = np.array(results[BESTKEY]['areas'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        delta_trans = np.array(results[BESTKEY]['delta_trans'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        fnames      = np.array(results[BESTKEY]['fnames'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        OBSNUM      = np.array(results[BESTKEY]['OBSNUM'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        UTDATE      = np.array(results[BESTKEY]['UTDATE'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        AZSTART     = np.array(results[BESTKEY]['AZSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        AZEND       = np.array(results[BESTKEY]['AZEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        ELSTART     = np.array(results[BESTKEY]['ELSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        ELEND       = np.array(results[BESTKEY]['ELEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        HUMSTART    = np.array(results[BESTKEY]['HUMSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        HUMEND      = np.array(results[BESTKEY]['HUMEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        BPSTART     = np.array(results[BESTKEY]['BPSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        BPEND       = np.array(results[BESTKEY]['BPEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        WNDSPDST    = np.array(results[BESTKEY]['WNDSPDST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        WNDSPDEN    = np.array(results[BESTKEY]['WNDSPDEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        WNDDIRST    = np.array(results[BESTKEY]['WNDDIRST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        WNDDIREN    = np.array(results[BESTKEY]['WNDDIREN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAU225ST    = np.array(results[BESTKEY]['TAU225ST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAU225EN    = np.array(results[BESTKEY]['TAU225EN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUDATST    = np.array(results[BESTKEY]['TAUDATST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        TAUDATEN    = np.array(results[BESTKEY]['TAUDATEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        SEEINGST    = np.array(results[BESTKEY]['SEEINGST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        SEEINGEN    = np.array(results[BESTKEY]['SEEINGEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        SEEDATST    = np.array(results[BESTKEY]['SEEDATST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        SEEDATEN    = np.array(results[BESTKEY]['SEEDATEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        FRLEGTST    = np.array(results[BESTKEY]['FRLEGTST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        FRLEGTEN    = np.array(results[BESTKEY]['FRLEGTEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        BKLEGTST    = np.array(results[BESTKEY]['BKLEGTST'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        BKLEGTEN    = np.array(results[BESTKEY]['BKLEGTEN'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]
        ...
        for i in range(len(np.array(results[BESTKEY]['OBSSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)])):
            OBSST.append(int(np.array(results[BESTKEY]['OBSSTART'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)][i].split('T')[1].split(':')[0]))
            OBSEN.append(int(np.array(results[BESTKEY]['OBSEND'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)][i].split('T')[1].split(':')[0]))
        ...
        for i in range(len(ATSTART)):
            ...
            if len(str(SEEDATST[i])) and str(SEEDATEN[i])[0]=="<":
        ...
        goodind = np.where(np.logical_and(airmasses<AMLIM,np.logical_and(np.logical_and(FWHM1s<FWHMLIM,FWHM2s<FWHMLIM),np.logical_and(np.logical_and((TAUST+TAUEN)/2.0>LOWERTAULIM,(TAUST+TAUEN)/2.0<UPPERTAULIM),np.logical_and(OBSST>=OBSSTLIM,OBSEN<OBSENLIM)))))

        peakfluxes=np.array(results[BESTKEY]['peak_fluxes'])[np.where(np.isnan(np.array(peakfluxes1))*-1+1>0)]#/np.average(np.array(results[BESTKEY]['peak_fluxes'])[np.where(np.isnan(np.array(peakfluxes))*-1+1>0)])
        
        for i in range(len(peakfluxes)):
            peakfluxes_all.append(peakfluxes[i])#/np.average(peakfluxes))
        ...
        if results[BESTKEY]['source'] == 'URANUS':
            ...
            peak850_actual_nogoodind   = []
            arcsec850_actual_nogoodind = []
            peak450_actual_nogoodind   = []
            arcsec450_actual_nogoodind = []
            aperture_cor_nogoodind     = []


            for eachfile in range(len(np.array(results[BESTKEY]['fnames'])[goodind])):
                if len(np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile].split('.'))>1:
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile]
                else:
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile]+'.00'
                ...
                aperture_cor.append(1.0)
                #aperture_cor.append(total_flux_cor(2*np.sqrt(np.array(results[BESTKEY]['areas'])[goodind][eachfile]/np.pi),wavelength))

            for eachfile in range(len(np.array(results[BESTKEY]['fnames']))):
                if len(np.array(results[BESTKEY]['OBSSTART'])[eachfile].split('.'))>1:
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[eachfile]
                else:
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[eachfile]+'.00'
                peak850_actual_nogoodind.append(fluxes.get_flux('URANUS',datestr_long,filter_=850).f_beam)
                arcsec850_actual_nogoodind.append(fluxes.get_flux('URANUS',datestr_long,filter_=850).f_total)
                peak450_actual_nogoodind.append(fluxes.get_flux('URANUS',datestr_long,filter_=450).f_beam)
                arcsec450_actual_nogoodind.append(fluxes.get_flux('URANUS',datestr_long,filter_=450).f_total)
                aperture_cor_nogoodind.append(1.0)
            ...
            aperture_cor_nogoodind     = np.array(aperture_cor_nogoodind)
            peak850_actual_nogoodind   = np.array(peak850_actual_nogoodind)
            arcsec850_actual_nogoodind = np.array(arcsec850_actual_nogoodind)
            peak450_actual_nogoodind   = np.array(peak450_actual_nogoodind)
            arcsec450_actual_nogoodind = np.array(arcsec450_actual_nogoodind)


            if wavelength == '850':
                peak_fcfs   = peak850_actual/peakfluxes[goodind]
                peak_fcfs_nogoodind = peak850_actual_nogoodind/peakfluxes
                arcsec_fcfs = (arcsec850_actual/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                arcsec_fcfs_nogoodind = (arcsec850_actual_nogoodind/((totalfluxes)*PIX_SIZE**2.0))
            if wavelength == '450':
                peak_fcfs   = peak450_actual/peakfluxes[goodind]
                peak_fcfs_nogoodind = peak450_actual_nogoodind/peakfluxes
                arcsec_fcfs = (arcsec450_actual/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                arcsec_fcfs_nogoodind = (arcsec450_actual_nogoodind/((totalfluxes)*PIX_SIZE**2.0))

        if results[BESTKEY]['source'] == 'MARS':
            ...
            for eachfile in range(len(np.array(results[BESTKEY]['fnames'])[goodind])):
                if len(np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile].split('.'))>1:
                    ...
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile]+'.00'
                peak850_actual.append(fluxes.get_flux('MARS',datestr_long,filter_=850).f_beam)
                arcsec850_actual.append(fluxes.get_flux('MARS',datestr_long,filter_=850).f_total)
                peak450_actual.append(fluxes.get_flux('MARS',datestr_long,filter_=450).f_beam)
                arcsec450_actual.append(fluxes.get_flux('MARS',datestr_long,filter_=450).f_total)
                aperture_cor.append(1.0)
                #aperture_cor.append(total_flux_cor(2*np.sqrt(np.array(results[BESTKEY]['areas'])[goodind][eachfile]/np.pi),wavelength))
            ...
            for eachfile in range(len(np.array(results[BESTKEY]['fnames']))):
                if len(np.array(results[BESTKEY]['OBSSTART'])[eachfile].split('.'))>1:
                    ...
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[eachfile]+'.00'
                peak850_actual_nogoodind.append(fluxes.get_flux('MARS',datestr_long,filter_=850).f_beam)
                arcsec850_actual_nogoodind.append(fluxes.get_flux('MARS',datestr_long,filter_=850).f_total)
                peak450_actual_nogoodind.append(fluxes.get_flux('MARS',datestr_long,filter_=450).f_beam)
                arcsec450_actual_nogoodind.append(fluxes.get_flux('MARS',datestr_long,filter_=450).f_total)
                aperture_cor_nogoodind.append(1.0)
        ...
        if results[BESTKEY]['source'] == 'NEPTUNE':
            ...
            for eachfile in range(len(np.array(results[BESTKEY]['fnames'])[goodind])):
                if len(np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile].split('.'))>1:
                    ...
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[goodind][eachfile]+'.00'
                peak850_actual.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=850).f_beam)
                arcsec850_actual.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=850).f_total)
                peak450_actual.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=450).f_beam)
                arcsec450_actual.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=450).f_total)
                aperture_cor.append(1.0)
                #aperture_cor.append(total_flux_cor(2*np.sqrt(np.array(results[BESTKEY]['areas'])[goodind][eachfile]/np.pi),wavelength))
            ...
            for eachfile in range(len(np.array(results[BESTKEY]['fnames']))):
                if len(np.array(results[BESTKEY]['OBSSTART'])[eachfile].split('.'))>1:
                    ...
                    datestr_long     = np.array(results[BESTKEY]['OBSSTART'])[eachfile]+'.00'
                peak850_actual_nogoodind.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=850).f_beam)
                arcsec850_actual_nogoodind.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=850).f_total)
                peak450_actual_nogoodind.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=450).f_beam)
                arcsec450_actual_nogoodind.append(fluxes.get_flux('NEPTUNE',datestr_long,filter_=450).f_total)
                aperture_cor_nogoodind.append(1.0)
        ...
        if results[BESTKEY]['source'] == 'CRL2688':
            ...
            if wavelength == '850':
                peak_fcfs   = CRL2688_peak_850/peakfluxes[goodind]
                arcsec_fcfs = (CRL2688_arcsec_850/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = CRL2688_peak_850/peakfluxes
                arcsec_fcfs_nogoodind = (CRL2688_arcsec_850/((totalfluxes)*PIX_SIZE**2.0))
            if wavelength == '450':
                peak_fcfs   = CRL2688_peak_450/peakfluxes[goodind]
                arcsec_fcfs = (CRL2688_arcsec_450/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = CRL2688_peak_450/peakfluxes
                arcsec_fcfs_nogoodind = (CRL2688_arcsec_450/((totalfluxes)*PIX_SIZE**2.0))

        if results[BESTKEY]['source'] == 'CRL618':
            ...
            if wavelength == '850':
                peak_fcfs   = CRL618_peak_850/peakfluxes[goodind]
                arcsec_fcfs = (CRL618_arcsec_850/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = CRL618_peak_850/peakfluxes
                arcsec_fcfs_nogoodind = (CRL618_arcsec_850/((totalfluxes)*PIX_SIZE**2.0))
            if wavelength == '450':
                peak_fcfs   = CRL618_peak_450/peakfluxes[goodind]
                arcsec_fcfs = (CRL618_arcsec_450/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = CRL618_peak_450/peakfluxes
                arcsec_fcfs_nogoodind = (CRL618_arcsec_450/((totalfluxes)*PIX_SIZE**2.0))

        if results[BESTKEY]['source'] == 'Arp220':
            ...
            if wavelength == '850':
                peak_fcfs   = ARP220_peak_850/peakfluxes[goodind]
                arcsec_fcfs = (ARP220_arcsec_850/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = ARP220_peak_850/peakfluxes
                arcsec_fcfs_nogoodind = (ARP220_arcsec_850/((totalfluxes)*PIX_SIZE**2.0))
            if wavelength == '450':
                peak_fcfs   = ARP220_peak_450/peakfluxes[goodind]
                arcsec_fcfs = (ARP220_arcsec_450/((totalfluxes[goodind]/aperture_cor)*PIX_SIZE**2.0))
                peak_fcfs_nogoodind = ARP220_peak_450/peakfluxes
                arcsec_fcfs_nogoodind = (ARP220_arcsec_450/((totalfluxes)*PIX_SIZE**2.0))

        for i in totalfluxes[goodind]/aperture_cor:
            # NORMALISE TOTAL FLUX TO AVERAGE SO WE ARE COMPARING APPLES TO APPLES WITH MULTIPLE SOURCES
            totalfluxes_all.append(i/np.average(totalfluxes[goodind][np.where(np.logical_and(np.isnan(totalfluxes[goodind])*-1+1>0,np.isnan(aperture_cor)*-1+1>0))]/aperture_cor[np.where(np.logical_and(np.isnan(totalfluxes[goodind])*-1+1>0,np.isnan(aperture_cor)*-1+1>0))]))

        for i in totalfluxes:
            totalfluxes_nogoodind_all.append(i)
            totalfluxes_nogoodind_all_norm.append(i/np.average(totalfluxes[np.where(np.isnan(totalfluxes)*-1+1>0)]))
        ...
        for eachpeakfcf in peak_fcfs_nogoodind:
            peak_fcfs_nogoodind_all.append(eachpeakfcf)

        for eacharcsecfcf in arcsec_fcfs_nogoodind:
            arcsec_fcfs_nogoodind_all.append(eacharcsecfcf)


        MJD_dates = []
        readable_time=[]
        for eachfile in np.array(results[BESTKEY]['fnames'])[goodind]:
            ...
            sourceind_all.append(results[BESTKEY]['source'])
            sourceindID_all.append(CALIBRATOR_SOURCE_ID[np.where(CALIBRATOR_SOURCES==results[BESTKEY]['source'])][0])

        MJD_dates_nogoodind = []
        readable_time=[]
        for eachfile in np.array(results[BESTKEY]['fnames']):
            date  = eachfile.split('_')[0].split('s')[1]
            year  = date[0:4]
            month = date[4:6]
            day   = date[6:8]
            datestr = year+'-'+month+'-'+day+'T'+'00:00:00'
            t = Time(datestr, format='isot', scale='utc')
            MJD_dates_nogoodind.append(t.mjd)
            MJD_dates_nogoodind_all.append(t.mjd)
            readable_time.append(date)
            sourceind_nogoodind_all.append(results[BESTKEY]['source'])
            sourceindID_nogoodind_all.append(CALIBRATOR_SOURCE_ID[np.where(CALIBRATOR_SOURCES==results[BESTKEY]['source'])][0])


    peak_fcfs    = np.array(peak_fcfs_all) # ALREADY HAS GOODIND APPLIED
    peak_fcfs_nogoodind = np.array(peak_fcfs_nogoodind_all)
    arcsec_fcfs  = np.array(arcsec_fcfs_all) # ALREADY HAS GOODIND APPLIED
    arcsec_fcfs_nogoodind = np.array(arcsec_fcfs_nogoodind_all)
    MJD_dates    = np.array(MJD_dates_all) # ALREADY HAS GOODIND APPLIED
    MJD_dates_nogoodind = np.array(MJD_dates_nogoodind_all)
    sourceind    = np.array(sourceind_all) # ALREADY HAS GOODIND APPLIED
    sourceind_nogoodind = np.array(sourceind_nogoodind_all)
    sourceindID    = np.array(sourceindID_all) # ALREADY HAS GOODIND APPLIED
    sourceindID_nogoodind = np.array(sourceindID_nogoodind_all)
    ...
    totalfluxes  =    np.array(totalfluxes_all) # ALREADY HAS GOODIND APPLIED
    totalfluxes_nogoodind = np.array(totalfluxes_nogoodind_all)
    totalfluxes_nogoodind_norm = np.array(totalfluxes_nogoodind_all_norm)
    ...
    # NOW MAKE THE MASTER CATALOGUE

    if present_epoch_only == 1:
        catalogue_file=open('master_catalogue_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'_'+present_epoch+'.txt','w')
    else:
        catalogue_file=open('master_catalogue_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'.txt','w')
    ...
    for eachfile in range(len(ATSTART)):
        ...
        catalogue_file.write(str(sourceind_nogoodind[eachfile])+'\t'+str(sourceindID_nogoodind[eachfile])+'\t'+str(UTDATE[eachfile])+'\t'+str(OBSNUM[eachfile])+'\t'+str(peakfluxes[eachfile])+'\t'+str(peakfluxes_norm[eachfile])+'\t'+str(totalfluxes_nogoodind[eachfile])+'\t'+str(totalfluxes_nogoodind_norm[eachfile])+'\t'+str(FWHM1s[eachfile])+'\t'+str(FWHM2s[eachfile])+'\t'+str(areas[eachfile])+'\t'+str(peak_fcfs_nogoodind[eachfile])+'\t'+str(arcsec_fcfs_nogoodind[eachfile])+'\t'+str(trans_witham[eachfile])+'\t'+str(zentrans[eachfile])+'\t'+str(MJD_OBS[eachfile])+'\t'+str(MJD_END[eachfile])+'\t'+str(OBSST[eachfile])+'\t'+str(OBSEN[eachfile])+'\t'+str(AMSTART[eachfile])+'\t'+str(AMEND[eachfile])+'\t'+str(ATSTART[eachfile])+'\t'+str(ATEND[eachfile])+'\t'+str(FRLEGTST[eachfile])+'\t'+str(FRLEGTEN[eachfile])+'\t'+str(BKLEGTST[eachfile])+'\t'+str(BKLEGTEN[eachfile])+'\t'+str(AZSTART[eachfile])+'\t'+str(AZEND[eachfile])+'\t'+str(ELSTART[eachfile])+'\t'+str(ELEND[eachfile])+'\t'+str(HUMSTART[eachfile])+'\t'+str(HUMEND[eachfile])+'\t'+str(BPSTART[eachfile])+'\t'+str(BPEND[eachfile])+'\t'+str(WNDSPDST[eachfile])+'\t'+str(WNDSPDEN[eachfile])+'\t'+str(WNDDIRST[eachfile])+'\t'+str(WNDDIREN[eachfile])+'\t'+str(TAUST[eachfile])+'\t'+str(TAUEN[eachfile])+'\t'+str(TAUST_T[eachfile])+'\t'+str(TAUEN_T[eachfile])+'\t'+str(TAU225ST[eachfile])+'\t'+str(TAU225EN[eachfile])+'\t'+str(TAUDATST[eachfile])+'\t'+str(TAUDATEN[eachfile])+'\t'+str(SEEINGST[eachfile])+'\t'+str(SEEINGEN[eachfile])+'\t'+str(SEEDATST[eachfile])+'\t'+str(SEEDATEN[eachfile])+'\t'+str(ELAPTIME[eachfile])+'\t'+str(fnames[eachfile])+'\t'+str(delta_trans[eachfile])+'\n')
       # print('#SOURCE\tSOURCE_ID\tUTDATE\tOBSNUM\tPEAK_FLUX\tNORM_PEAK_FLUX\tTOTAL_FLUX\tNORM_TOTAL_FLUX\tFWHM1\tFWHM2\tAREA\tPEAK_FCF\tARCSEC_FCF\tTRANS\tZEN_TRANS\tMJDST\tMJDEN\tOBSST\tOBSEN\tAMSTART\tAMEND\tATSTART\tATEND\tFRLEGTST\tFRLEGTEN\tBKLEGTST\tBKLEGTEN\tAZSTART\tAZEND\tELSTART\tELEND\tHUMSTART\tHUMEND\tBPSTART\tBPEND\tWNDSPDST\tWNDSPDEN\tWNDDIRST\tWNDDIREN\tWVMTAUST\tWVMTAUEN\tWVMTAUST_TIME\tWVMTAUEN_TIME\tSMATAU225ST\tSMATAU225EN\tSMATAU225ST_TIME\tSMATAU225EN_TIME\tSEEINGST\tSEEINGEN\tSEEDATST\tSEEDATEN\tELAPTIME\tFNAME\tDIFF_TRANSOBS_TRANSMODEL')
        #print(str(sourceind_nogoodind[eachfile])+'\t'+str(sourceindID_nogoodind[eachfile])+'\t'+str(UTDATE[eachfile])+'\t'+str(OBSNUM[eachfile])+'\t'+str(peakfluxes[eachfile])+'\t'+str(peakfluxes_norm[eachfile])+'\t'+str(totalfluxes_nogoodind[eachfile])+'\t'+str(totalfluxes_nogoodind_norm[eachfile])+'\t'+str(FWHM1s[eachfile])+'\t'+str(FWHM2s[eachfile])+'\t'+str(areas[eachfile])+'\t'+str(peak_fcfs_nogoodind[eachfile])+'\t'+str(arcsec_fcfs_nogoodind[eachfile])+'\t'+str(trans_witham[eachfile])+'\t'+str(zentrans[eachfile])+'\t'+str(MJD_OBS[eachfile])+'\t'+str(MJD_END[eachfile])+'\t'+str(OBSST[eachfile])+'\t'+str(OBSEN[eachfile])+'\t'+str(AMSTART[eachfile])+'\t'+str(AMEND[eachfile])+'\t'+str(ATSTART[eachfile])+'\t'+str(ATEND[eachfile])+'\t'+str(FRLEGTST[eachfile])+'\t'+str(FRLEGTEN[eachfile])+'\t'+str(BKLEGTST[eachfile])+'\t'+str(BKLEGTEN[eachfile])+'\t'+str(AZSTART[eachfile])+'\t'+str(AZEND[eachfile])+'\t'+str(ELSTART[eachfile])+'\t'+str(ELEND[eachfile])+'\t'+str(HUMSTART[eachfile])+'\t'+str(HUMEND[eachfile])+'\t'+str(BPSTART[eachfile])+'\t'+str(BPEND[eachfile])+'\t'+str(WNDSPDST[eachfile])+'\t'+str(WNDSPDEN[eachfile])+'\t'+str(WNDDIRST[eachfile])+'\t'+str(WNDDIREN[eachfile])+'\t'+str(TAUST[eachfile])+'\t'+str(TAUEN[eachfile])+'\t'+str(TAUST_T[eachfile])+'\t'+str(TAUEN_T[eachfile])+'\t'+str(TAU225ST[eachfile])+'\t'+str(TAU225EN[eachfile])+'\t'+str(TAUDATST[eachfile])+'\t'+str(TAUDATEN[eachfile])+'\t'+str(SEEINGST[eachfile])+'\t'+str(SEEINGEN[eachfile])+'\t'+str(SEEDATST[eachfile])+'\t'+str(SEEDATEN[eachfile])+'\t'+str(ELAPTIME[eachfile])+'\t'+str(fnames[eachfile])+'\t'+str(delta_trans[eachfile])+'\n')

    catalogue_file.close()

    goodind      = np.where(np.logical_and(airmasses<AMLIM,np.logical_and(np.logical_and(FWHM1s<FWHMLIM,FWHM2s<FWHMLIM),np.logical_and(np.logical_and((TAUST+TAUEN)/2.0>LOWERTAULIM,(TAUST+TAUEN)/2.0<UPPERTAULIM),np.logical_and(OBSST>=OBSSTLIM,OBSEN<OBSENLIM)))))

    peak_fcfs_goodind    = np.array(peak_fcfs_all) # ALREADY HAS GOODIND APPLIED
    arcsec_fcfs_goodind  = np.array(arcsec_fcfs_all) # ALREADY HAS GOODIND APPLIED
    MJD_dates_goodind    = np.array(MJD_dates_all) # ALREADY HAS GOODIND APPLIED
    sourceind_goodind    = np.array(sourceind_all) # ALREADY HAS GOODIND APPLIED
    sourceindID_goodind  = np.array(sourceindID_all) # ALREADY HAS GOODIND APPLIED
    totalfluxes_goodind  =    np.array(totalfluxes_all) # ALREADY HAS GOODIND APPLIED
    totalfluxes_norm_goodind = np.array(totalfluxes_nogoodind_all_norm)[goodind]
    zentrans_goodind     = np.array(zentrans_all)[goodind]
    trans_witham_goodind = np.array(transwitham_all)[goodind]            
    airmasses_goodind    = np.array(airmasses_all)[goodind]
    ATSTART_goodind      =    np.array(ATSTART_all)[goodind]
    ATEND_goodind        =    np.array(ATEND_all)[goodind]    
    AMSTART_goodind      =    np.array(AMSTART_all)[goodind]   
    AMEND_goodind        =    np.array(AMEND_all)[goodind]
    TAUST_goodind        =    np.array(TAUST_all)[goodind]
    TAUEN_goodind        =    np.array(TAUEN_all)[goodind] 
    TAUST_T_goodind      =    np.array(TAUST_T_all)[goodind] 
    TAUEN_T_goodind      =    np.array(TAUEN_T_all)[goodind]
    FWHM1s_goodind       =    np.array(FWHM1s_all)[goodind] 
    FWHM2s_goodind       =    np.array(FWHM2s_all)[goodind]
    areas_goodind        =    np.array(areas_all)[goodind]
    delta_trans_goodind  =    np.array(delta_trans_all)[goodind]  
    OBSST_goodind        =    np.array(OBSST_all)[goodind]
    OBSEN_goodind        =    np.array(OBSEN_all)[goodind]
    peakfluxes_goodind   =    np.array(peakfluxes_all)[goodind]
    peakfluxes_norm_goodind = np.array(peakfluxes_all_norm)[goodind]
    fnames_goodind       = np.array(fnames_all)[goodind] 
    OBSNUM_goodind       = np.array(OBSNUM_all)[goodind] 
    UTDATE_goodind       = np.array(UTDATE_all)[goodind] 
    AZSTART_goodind      = np.array(AZSTART_all)[goodind] 
    AZEND_goodind        = np.array(AZEND_all)[goodind] 
    ELSTART_goodind      = np.array(ELSTART_all)[goodind] 
    ELEND_goodind        = np.array(ELEND_all)[goodind] 
    HUMSTART_goodind     = np.array(HUMSTART_all)[goodind] 
    HUMEND_goodind       = np.array(HUMEND_all)[goodind] 
    BPSTART_goodind      = np.array(BPSTART_all)[goodind] 
    BPEND_goodind        = np.array(BPEND_all)[goodind]  
    WNDSPDST_goodind     = np.array(WNDSPDST_all)[goodind] 
    WNDSPDEN_goodind     = np.array(WNDSPDEN_all)[goodind] 
    WNDDIRST_goodind      = np.array(WNDDIRST_all)[goodind] 
    WNDDIREN_goodind     = np.array(WNDDIREN_all)[goodind] 
    TAU225ST_goodind     = np.array(TAU225ST_all)[goodind] 
    TAU225EN_goodind     = np.array(TAU225EN_all)[goodind] 
    TAUDATST_goodind     = np.array(TAUDATST_all)[goodind] 
    TAUDATEN_goodind     = np.array(TAUDATEN_all)[goodind] 
    SEEINGST_goodind     = np.array(SEEINGST_all)[goodind] 
    SEEINGEN_goodind     = np.array(SEEINGEN_all)[goodind] 
    SEEDATST_goodind     = np.array(SEEDATST_all)[goodind] 
    SEEDATEN_goodind     = np.array(SEEDATEN_all)[goodind] 
    FRLEGTST_goodind     = np.array(FRLEGTST_all)[goodind] 
    FRLEGTEN_goodind     = np.array(FRLEGTEN_all)[goodind] 
    BKLEGTST_goodind     = np.array(BKLEGTST_all)[goodind] 
    BKLEGTEN_goodind     = np.array(BKLEGTEN_all)[goodind] 
    MJD_OBS_goodind      = np.array(MJD_OBS_all)[goodind] 
    MJD_END_goodind      = np.array(MJD_END_all)[goodind] 
    ELAPTIME_goodind     = np.array(ELAPTIME_all)[goodind] 


    if present_epoch_only == 1:
        catalogue_file_goodind = open('master_catalogue_constrained_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'_'+present_epoch+'.txt','w')
    else:
    ...
    if present_epoch_only == 1:
        combined_file = open('master_catalogue_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'_'+'combined.txt','w')
        # Concatenate Text Files:
        with open('master_catalogue_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'_'+present_epoch+'.txt') as f:
            present_epoch_lines = f.readlines()
        for i in present_epoch_lines:
            combined_file.write(i)

        with open('master_catalogue_'+wavelength+'_'+str(results[BESTKEY]['coeff1'])+'_'+str(results[BESTKEY]['coeff2'])+'_until_'+present_epoch+'.txt') as g:
            past_epochs_lines = g.readlines()
        for j in past_epochs_lines:
